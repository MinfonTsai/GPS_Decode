...
void nmea_process(char *buf)
{
  int i, num, gps, res,tt, ss, mm;
	  float latitude;
     char str[100],time[3];
					...
					char c = USART_ReceiveData(USART2);        
					 ...
					 if(c != '\r' && c != '\n' && (cnt < MAX_STRLEN) ){
							...
							cnt++;
							        ...
							        GPS_analysis( received_string);   
									//		nmea_process( received_string);
...
char time[100];
char latiplace1[100], longiplace1[100];
char latiplace2[100] , longiplace2[100];
//char  da[3],mo[3],ye[3];

void GPS_analysis( char *buf)
{

	   int  res, i, num, gps=1, tt , ss, mm;
	...
	//USARTx_Puts( USART1, buf );
	   ...
	   if((buf[3]=='G')&&(buf[4]=='G')&&(buf[5]=='A')) //Capture $GPGGA to convert
		{
			//USARTx_Puts( USART1, " GGA  " );
			num=0;
			for(i=0;i < res;i++) 
			{
				if(buf[i]!=',') { continue; } //If not£¬Jump back to  for loop i++
				switch(num){

					case 6:
						//
						if(buf[i+1]=='0') { //if 0£¬that is success
		...
		for(i=0;i<res;i++) {
			...
			switch(num) {
			   ...
			   case 1://Time 
				...
				time[1]=buf[i+2];
				time[2]='\0';
				tt=atoi(time)+8; //GMT+8  time[0]= buf[i+3];
				 
				 USARTx_Puts( USART1,  time  ); USARTx_Puts( USART1,  "/"  );
				 time[0]= buf[i+3];
				time[1]= buf[i+4];
				time[2]='\0';
			 mm= atoi(time); //MINUTE
			...
			case 2://
			    ...
			    USARTx_Puts( USART1, " -> " );  USARTx_Puts( USART1, latiplace1 ); USARTx_Puts( USART1, latiplace2 ); 
			
				 break;
			
			   case 3:  //
			if(buf[i+1]=='N') {  // lati = "North Latitude";  
				}
			else {  //lati = "South Latitude";  
				}
			break;
	
			case 4://
					longiplace1[0] = buf[i+1];//

					longiplace1[1] = buf[i+2];//
					longiplace1[2] = buf[i+3];//
					longiplace1[3] = '\0';
					longiplace2[0] = buf[i+4];//
					longiplace2[1] = buf[i+5];//
					longiplace2[2] = buf[i+6]; //buf[i+6] .
					longiplace2[3] = buf[i+7];//
					longiplace2[4] = buf[i+8];//
					longiplace2[5] = buf[i+9];//
					longiplace2[6] = buf[i+10];//
					longiplace2[7] = '\0';
					longitude = atof(longiplace1) +atof(longiplace2)/60;
			    USARTx_Puts( USART1, "  - " );  USARTx_Puts( USART1, longiplace1 );            USARTx_Puts( USART1, longiplace2 );  USARTx_Puts( USART1, " \r\n" );
					break;
			...
			case 5://
				if(buf[i+1]=='E') 
					...
					break;
				  case 6:
						  //printf("Taiwan time: %d:%d:%d ,%s: %f ,%s %f\n",tt,mm,ss,lati,latitude,longi,longitude);
						break;
			} //switch
		} //for(i=0;i<res;i++)
	}//if(gps!=0)
   ...
   while(*str)
    {
       USART_SendData(USARTx, *str++);
			//    GPIO_SetBits(GPIOB,GPIO_Pin_5);  //LED is transactive
...
void GPIOB_Init(void) {
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE); //Enable PB2 Peripheral clock

	
	GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_13; //GPIOB  P13
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP; //output push-pul	
